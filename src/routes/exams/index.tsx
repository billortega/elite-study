...
import {
  ArrowRight, BookOpen, Users, Phone, MessageCircle, ChevronLeft, Sparkles, Clock, Target,
} from "lucide-react";
  ...
  head: () => ({
    meta: [
      { title: "Exam Tracks | Super Digital Library Kanpur" },
      {
        name: "description",
        content:
          "Super Digital Library Kanpur supports aspirants preparing for UPSC, SSC, RRB, NEET, JEE, NDA, NIMCET, UPSI, AFCAT, Judiciary and more.",
      },
    ],
  }),
...
const EXAMS = [
  {
    ...
    subtitle: "IAS · IPS · IFS · IRS and 20+ Services",
    count: 2,
  ...
  },
  {
    ...
    subtitle: "MBBS · BDS · BAMS · BHMS · BVSc",
    count: 3,
  ...
  },
  {
    ...
    subtitle: "Army · Navy · Air Force Officer",
    count: 1,
    color: "oklch(0.60 0.26 285)",
    duration: "6–12 months",
    level: "National",
  },
  {
    ...
    subtitle: "MCA at 30+ NITs across India",
    count: 1,
    color: "oklch(0.65 0.20 200)",
    duration: "6–12 months",
    level: "National",
  },
  {
    ...
    subtitle: "Sub-Inspector in Uttar Pradesh Police",
    count: 3,
  ...
  },
  {
    ...
    subtitle: "Flying · Technical · Ground Duty",
    count: 1,
  ...
  },
  {
    ...
    subtitle: "Civil Judge (Junior Division)",
    count: 1,
  ...
  },
  {
    ...
    subtitle: "All streams, all goals welcome",
    count: 5,
    color: "oklch(0.60 0.26 285)",
    duration: "Your pace",
    level: "All levels",
...
function ExamsIndexPage() {
  const total = EXAMS.reduce((a, e) => a + e.count, 0);
  return (
        ...
        <div className="container-px mx-auto max-w-7xl text-center">
          ...
          <Reveal delay={0.12}>
            <p className="mx-auto mt-7 max-w-xl text-balance text-base text-muted-foreground sm:text-lg leading-relaxed">
              Super Digital Library supports aspirants across every major competitive exam in India. Click any exam to explore full details, syllabus, and tips.
            </p>
          </Reveal>
          <Reveal delay={0.18}>
            <div className="mx-auto mt-10 inline-flex items-center gap-3 rounded-2xl border border-border bg-card/60 px-6 py-3.5 backdrop-blur-md">
              ...
              <span className="font-stat text-2xl font-bold text-accent">{total}</span>
      ...
      <section className="py-20 md:py-28">
        <div className="container-px mx-auto max-w-7xl">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {EXAMS.map((exam, i) => (
              <Reveal key={exam.slug} delay={i * 0.04} className={exam.isAny ? "sm:col-span-2 lg:col-span-3" : ""}>
                <Link
                  to={`/exams/${exam.slug}`}
                  className={`group relative flex flex-col overflow-hidden rounded-2xl border p-6 md:p-7 transition-all duration-300 hover:-translate-y-1 ${
                    exam.isAny
                      ? "border-accent/30 bg-accent/8 hover:border-accent/50 hover:shadow-lg hover:shadow-accent/10"
                      : "border-border bg-card hover:border-accent/30 hover:shadow-lg hover:shadow-black/20"
                  }`}
                >
                  {/* Top row */}
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      <div
                        ...
                        style={{ color: exam.isAny ? "oklch(0.60 0.26 285)" : "oklch(0.92 0 0)" }}
                      >
                        {exam.code}
                      </div>
                    ...
                    {/* Target icon */}
                    <div
                      className="mt-1 size-8 rounded-full flex items-center justify-center border transition-colors"
                      style={{ borderColor: `color-mix(in oklab, ${exam.color} 30%, transparent)`, background: `color-mix(in oklab, ${exam.color} 10%, transparent)` }}
                    >
                      <Target className="size-3.5" style={{ color: exam.color }} />
                    </div>
                  </div>

                  {/* Subtitle */}
                  <div className="mt-3 text-[13px] font-medium" style={{ color: exam.color }}>{exam.subtitle}</div>

                  {/* Meta */}
                  <div className="mt-4 flex items-center gap-4 text-[11px] text-muted-foreground">
                    <span className="flex items-center gap-1.5"><Clock className="size-3" /> {exam.duration}</span>
                    <span className="flex items-center gap-1.5"><Users className="size-3" />
                      <span style={{ color: exam.color }}>{exam.count}</span> studying now
                    </span>
                  </div>
                  ...
                  <div className="mt-5 flex items-center gap-1.5 text-sm font-medium text-muted-foreground group-hover:text-foreground transition-colors">
                    Aspirants studying here
                    <ArrowRight className="size-3.5 transition-transform group-hover:translate-x-1" />
                  </div>

                  {/* Hover glow */}
                  <div
                    className="pointer-events-none absolute -right-8 -top-8 size-32 rounded-full opacity-0 blur-2xl transition-all duration-500 group-hover:opacity-100"
                    style={{ background: `color-mix(in oklab, ${exam.color} 15%, transparent)` }}
                  />
                </Link>
              </Reveal>
            ))}
        ...
        <div className="container-px mx-auto max-w-3xl text-center relative z-10">
          <Reveal>
            ...
            <p className="mt-5 text-muted-foreground">
              Join serious aspirants who study consistently at Super Digital Library, Kanpur.
            </p>
